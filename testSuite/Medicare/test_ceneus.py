...
@allure.story('Visit medicare census page')
class TestCensus:
    # @pytest.fixture(scope='function', autouse=True)

    with allure.step(title='Open medicare census page'):
...
if __name__ == '__main__':
    # pytest.main(['testSuite/Medicare/test_census.py'])
    now = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time()))
    pytest.main("-s -q testSuite/Medicare/test_census.py  --alluredir results".format(now))
